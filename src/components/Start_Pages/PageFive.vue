<template>
    <div id="page">
        <h2>关于我们</h2>
        <div id="QAlist" class="normalScroll" @wheel="handleScroll">
            <ul>
                <li class="QAblock" v-for="item in QA" :key="item.question">
                    <p class="QAq">{{ item.question }}</p>
                    <p class="QAa" v-if="Array.isArray(item.answer)">
                    <ul>
                        <li v-for="subItem in item.answer" :key="subItem">{{ subItem }}</li>
                    </ul>
                    </p>
    ...
    data() {
        return {
            ...
            QA: [
                ...
                {
                    question: '这是什么项目?',
                    answer: '一个某学院自动运动的项目,是wsb的后续版本,我叫它wsn,这包含前端和后端两部分,分别是wsn(vite)和wsn(springboot),其中wsn(vite)是开源的,位于https://github.com/xmexg/wsv'
                },
                {
                ...
                {
                    question: '为什么要另辟蹊径?',
                    answer: '旧项目不想维护了,实际上wsb早已消极更新,最近3次更新日期分别是2023年6月7日、2023年9月6日、2024年6月8日。'
                },
                {
                    question: '为什么收费?',
                ...
                {
                    question: '机刷靠谱吗?',
                    answer: ['如果不是作死设置完全不合理的跑步数据,是不会被查出的。', '1. 我们完全逆向了学校跑步软件,机刷数据和真人跑数据格式完全一致,所有数据均可更改,对于一些明确定义的常量,wsb默认禁止修改,wsb前端源码完全公开没有任何加密,全部参数均在用户电脑生成,页面上所有不可编辑的输入框,均可通过删除disable属性解锁编辑,这是我们故意设计的,对于极客可完全查看并修改网站源码,生成任意格式的数据包。', '2.我们还研究了学校服务器,自wsb0.0.9起实现了复用任意服务器中已存在的跑步数据,我们建议的使用方法是打听到真人跑的学生学号,然后复用该学号的跑步数据,数据敏感信息会在wsb和wsn后台处理,不会被学校查出。']
                },
                {
                    question: 'wsb和wsn有什么区别?',
                    answer: 'wsn是wsb的全新版本,代码完全重写,目前通过复用过去两年来的优秀数据集,wsn解决了wsb不能自动刷的缺点,在未来可能会训练ai,自动生成跑步路线,从根本上解决跑步路线。'
                },
                {
                    question: '我(们)的其他业务?',
                    answer: '正在考虑图片视频ai换脸、ai换声,进一步应对晚间合拍查寝,需要借助在线算力平台,价格应该不会便宜。'
                },
                {
                    question: '蓝粉白?',
                    answer: '仅仅沿用wsb的色调,这和蓝粉白没有任何关系。'
                },
                {
                    question: '为什么“自动运动”需要登录，而“手动运动”不需要登录？',
                    answer: '软件在服务器检查用户是否已购买服务，理论上本身不需要登录。“自动运动”和“手动运动”的检查点不同，“自动运动”设定为需要登录仅仅提高极客穷举难度。'
                },
                {
                    question: '为什么不提供学校的密码也能运动？',
                    answer: '学校服务器跑步鉴权方式和密码无关，不需要密码也能正常运动。即便在手机app中，也能通过hook的方式实现任意账号免密码登录。'
                },
                {
                    question: '这是秒刷还是慢刷？',
...
/* 复制于 https://codepen.io/shuangcs/pen/gKbQEj */
#stars {
    width: 1px;
    height: 1px;
